def tick args
  ...
  if args.state.phase == :selecting_top_level_action
    ...
    elsif args.inputs.keyboard.key_down.left || args.inputs.keyboard.key_down.right
      case args.state.potential_action
      when :attack
        args.state.potential_action = :magic
      when :special
  ...
  if args.state.phase == :selecting_target
    ...
    if args.inputs.keyboard.key_down.enter
      damage = deal_damage args
      args.state.enemies[args.state.potential_enemy_index].hp = args.state.enemies[args.state.potential_enemy_index].hp - damage
...
def render_actions_menu args
  if !args.state.selected_action
    selected_rect = if args.state.potential_action == :attack
                      args.layout.rect(row:  8, col: 4, w: 4, h: 1)
                    elsif args.state.potential_action == :special
                      args.layout.rect(row:  9, col: 4, w: 4, h: 1)
                    elsif args.state.potential_action == :magic
                      args.layout.rect(row: 8, col: 8, w: 4, h: 1)
                    elsif args.state.potential_action == :items
                      args.layout.rect(row: 9, col: 8, w: 4, h: 1)
                    end

    args.outputs.solids  << selected_rect.merge(r: 200, g: 200, b: 200)
  end

  args.outputs.borders << args.layout.rect(row:  8, col: 4, w: 4, h: 1)
  args.outputs.labels  << args.layout.rect(row:  8, col: 4, w: 4, h: 1).center.merge(text: "Attack", vertical_alignment_enum: 1, alignment_enum: 1)

  args.outputs.borders << args.layout.rect(row:  9, col: 4, w: 4, h: 1)
  args.outputs.labels  << args.layout.rect(row:  9, col: 4, w: 4, h: 1).center.merge(text: "Special", vertical_alignment_enum: 1, alignment_enum: 1)

  args.outputs.borders << args.layout.rect(row: 8, col: 8, w: 4, h: 1)
  args.outputs.labels  << args.layout.rect(row: 8, col: 8, w: 4, h: 1).center.merge(text: "Magic", vertical_alignment_enum: 1, alignment_enum: 1)

  args.outputs.borders << args.layout.rect(row: 9, col: 8, w: 4, h: 1)
  args.outputs.labels  << args.layout.rect(row: 9, col: 8, w: 4, h: 1).center.merge(text: "Items", vertical_alignment_enum: 1, alignment_enum: 1)
end
  ...
  args.outputs.primitives << args.state.enemies.map_with_index do |e, i|
    if e.dead
      ...
      [
        ...
        args.layout.rect(row: 2, col: 9 + i * 2, w: 2, h: 2).center.label!(text: "#{e.hp}", vertical_alignment_enum: 2, alignment_enum: 2)
      ]
    else
      [
        args.layout.rect(row: 1, col: 9 + i * 2, w: 2, h: 2).border!,
